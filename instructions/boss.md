...
- worker1: `../aboutme-worker1`（ブランチ `worker1-work`）
- worker2: `../aboutme-worker2`（ブランチ `worker2-work`）
- worker3: `../aboutme-worker3`（ブランチ `worker3-work`）
...
使用する各workerについて実行:
```bash
git worktree add -b worker{N}-work ../aboutme-worker{N} main
```
（既にブランチ/worktreeが残っている場合は `git worktree remove ../aboutme-worker{N} --force` と `git branch -D worker{N}-work` で先に片付ける）
...
```bash
./agent-send.sh worker1 "あなたはworker1です。../aboutme-worker1 に移動し、./tmp/tasks/worker1.md の内容に従って作業してください"
```
...
### 8. 後片付け
```bash
git worktree remove ../aboutme-worker{N} --force
