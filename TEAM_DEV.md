...
- ⚠️ 記憶はセッションを起動したディレクトリ単位。`~/projects/aboutMe`など特定プロジェクトの中で`claude`を起動すると別スコープ扱いになり、この記憶は自動では読み込まれない。プロジェクト横断の相談は`~`や`~/projects`から起動すること
...
- `multiagent-aboutme` と `president-aboutme` が**両方表示された** → そのまま「[セッションにアタッチ](#3-セッションにアタッチcd不要どこからでも実行可)」へ
- **表示されない/片方だけ** → 次の「2. 初回 or セッションが消えたとき」へ

### 2. 初回 or セッションが消えたとき（要cd）

```bash
cd ~/projects/aboutMe   # まだこのディレクトリにいなければ
./start.sh
```
...
```bash
tmux attach-session -t president-aboutme
tmux attach-session -t multiagent-aboutme
```
...
```text
📊 president-aboutme セッション (1ペイン)
└── PRESIDENT: 要件をヒアリングし、boss1に発注する統括責任者

📊 multiagent-aboutme セッション (4ペイン)
...
3. boss1: 要件を最大3タスクに分割し、workerごとにgit worktreeを作成
   (worker1: ../aboutMe-worker1 ブランチworker1-work、worker2/3も同様)
...
⚠️ worktreeへの`cd`はラウンドごとに毎回発生する: boss1はマージ後に各workerのworktreeを削除し(後片付け)、次のタスクラウンドでは作り直す。そのためboss1→workerの指示メッセージには毎回`../aboutMe-workerN に移動し`が含まれ、workerは指示を受けるたびに`cd`から作業を始める(`instructions/worker.md`手順1)。
...
`./`で始まるコマンドと`cat ./tmp/...`は要cd(`cd ~/projects/aboutMe`)。`tmux`コマンドはcd不要。
...
tmux list-panes -t multiagent-aboutme

# 環境リセット(タスク管理ファイル・worker用worktree/branchも自動でクリアされる)
tmux kill-session -t multiagent-aboutme
tmux kill-session -t president-aboutme
./setup.sh
```
