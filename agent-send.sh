    ...
    case "$1" in
        ...
        "boss1"|"worker1"|"worker2"|"worker3")
            ...
            if tmux has-session -t "multiagent-aboutme" 2>/dev/null; then
                ...
                # pane番号を計算
                case "$1" in
                    "boss1") echo "multiagent-aboutme:$window_name.$((pane_index))" ;;
                    "worker1") echo "multiagent-aboutme:$window_name.$((pane_index + 1))" ;;
                    "worker2") echo "multiagent-aboutme:$window_name.$((pane_index + 2))" ;;
                    "worker3") echo "multiagent-aboutme:$window_name.$((pane_index + 3))" ;;
                esac
            else
                echo ""
            fi
...
show_agents() {
    ...
    # presidentセッション確認
    if tmux has-session -t "president-aboutme" 2>/dev/null; then
